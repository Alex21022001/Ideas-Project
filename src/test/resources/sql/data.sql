...
VALUES (1, 'test1', 'test1-description', 'test1.png', 'test1.pdf', '2023-07-19 08:49:23.075000',
        (SELECT u.id FROM users u WHERE id = 1)),
       (2, 'test2', 'test2-description', 'test2.png', 'test2.pdf', '2023-07-18 08:49:23.075000',
        (SELECT u.id FROM users u WHERE id = 1)),
       (3, 'test3', 'test3-description', 'test3.png', 'test3.pdf', '2023-07-16 08:49:23.075000',
        (SELECT u.id FROM users u WHERE id = 2));
SELECT setval('project_id_seq', (SELECT MAX(id) FROM project));

INSERT INTO project_status(id, status, project_id, version)
VALUES (1, 'IN_PROGRESS', 1, 0),
       (2, 'REJECTED', 2, 1),
       (3, 'ACCEPTED', 3, 1);
